...
public:
    ...
    explicit SearchServer(const StringContainer& stop_words) {
        if (all_of(stop_words.begin(), stop_words.end(), IsValidWord)) {
            ...
            throw invalid_argument("Слово содержит спецсимволы"s);
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if (document_id < 0) {
            throw invalid_argument("Документ не был добавлен, так его id отрицательный"s);
        }
        if (documents_.count(document_id)) {
            throw invalid_argument("Документ не был добавлен, так его id совпадает с уже имеющимся"s);
        }
